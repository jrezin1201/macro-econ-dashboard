 ...
 *
 * Shows debt trends, share buybacks, dilution, dividends, and capital allocation policy.
 */
...
export async function generateMetadata(props: Props) {
  ...
  return {
    title: `${ticker.toUpperCase()} Capital Allocation | Company Analysis`,
  };
}

export default async function CapitalAllocationPage(props: Props) {
  const { ticker } = await props.params;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="p-12 bg-yellow-500/10 border border-yellow-500/20 rounded-lg text-center">
          <h2 className="text-2xl font-bold text-yellow-200 mb-4">
            Capital Allocation Analysis Coming Soon
          </h2>
          <p className="text-yellow-200/70">
            This tab will show debt trends, buybacks, dilution, SBC, and capital discipline scores.
          </p>
        </div>
      </div>
    </div>
